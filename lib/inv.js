            ...
            if (descriptions && assets) {
                async.forEach(descriptions, (description, cbDesc) => async.forEach(assets, (asset, cbAsset) => {
                    if (
                        ...
                        if (description.actions[i].name.indexOf('Inspect') !== -1) {
                            const inspectURL = Trade.prototype.getInspect(steamID64, asset.assetid, description.actions)
                            inventory[asset.assetid].inspect = inspectURL
                        }
...
Trade.prototype.getInspect = function getInspect(steam64, assetid, data) {
    data = data.inspect[0].link;
    data = data.replace('%owner_steamid%', steam64);
    data = data.replace('%assetid%', item.assetid);
    return data;
}
