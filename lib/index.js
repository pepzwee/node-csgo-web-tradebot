...
Trade.prototype.getIgnoreItems = function(name) {
    var trash = ["music kit","Sticker"];
	var flag = 1;
	console.log(name);
	for (var i=0;i<=trash.length;i++) {
	if ( ~name.indexOf(trash[i]))
      {
	  flag = 0;
	  }
            ...
            if( ! err) {
                ...
                for(let i in userInventory) {
                    let item = userInventory[i];
					 let flag = self.getPrice(item.data.market_hash_name, 'user', item.item_type);
					 let flagtrash =  self.getIgnoreItems(item.data.market_hash_name);
					 console.log(flagtrash);
                    if(obj.user.indexOf(item.assetid) !== -1 && flag!=-1 && flagtrash!=0 ) {
