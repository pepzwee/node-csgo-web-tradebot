...
Trade.prototype.startBots = function startBots(done) {
    ...
    async.eachOfSeries(config.bots, (bot, id, callback) => {
        ...
        self.instances[id].client.addListener('webSession', (sessionID, cookies) => {
            ...
            self.instances[id].client.setPersona(SteamUser.EPersonaState.LookingToTrade, bot.personaName)
            // Initialize CS:GO
            self.instances[id].client.gamesPlayed([config.appID])
        })
        self.instances[id].manager.on('pollData', (data) => {
            fs.mkdir('./polls', (err) => {
                if (err) throw Error(err, console.log('Failed to create polls folder, please create one manually'))
            })
            fs.writeFile(`./polls/${id}.json`, JSON.stringify(data), (err) => {
                if (err) throw new Error(err, console.log('Failed to create bots json file, create one manually.'))
            })
        })
