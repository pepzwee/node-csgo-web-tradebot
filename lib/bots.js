...
Trade.prototype.getFloatsFromTrade = function getFloatsFromTrade(instanceID, items) {
    const self = this
    const instance = self.instances[instanceID]
    if (!items || !Object.keys(items).length) {
            console.log('[FloatChecker:Error]', `Could not get items for "${instanceID}", empty trade?`)
            return;
    }
    const csgo = instance.csgo
    async.eachOfSeries(items, (item, assetid, itemCallback) => {
        const csgo = instance.csgo
        let inspectLink = Trade.prototype.getInspect(instanceID, item.assetid, item.actions)
        if (typeof self.floats[item.assetid] !== 'undefined' || inspectLink == null ) {
            return itemCallback()
        }
        csgo.inspectItem(inspectLink);
        setTimeout(itemCallback, 3000)
    }, () => {
        console.log('[FloatChecker:Progress]', 'Done floats for trade.')
    })
}
